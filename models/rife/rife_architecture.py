...
class IFBlock(nn.Module):
    ...
    def forward(self, x, flow, scale=1):
        ...
        # Let's go back to basics - maybe the issue is simpler
        # Standard RIFE should output 5 channels: 4 for flow + 1 for mask
        # But we have 24. Let's try just taking first 5 and see what happens
        
        flow = tmp[:, :4] * scale * 2
        mask = tmp[:, 4:5]
        
        # But clip flow to reasonable range to avoid extreme values
        flow = torch.clamp(flow, -100, 100)  # Clamp to reasonable flow range
